...
while true; do
    ...
    # Reposicionar cursor arriba sin limpiar pantalla (Sin parpadeo)
    printf "\033[1;1H"

    # Redibujar interfaz
    printf "%b┌───────────────────────────────────────────────────────────────────────────┐%b\n" "$D" "$N"
    printf "%b│%b  %b[ %b⚡ KIRA-SSH%b ]%b  🔐 %bGENERADOR DE CUENTAS DEMO | KIRA VIP%b             %b│%b\n" "$D" "$N" "$D" "$C" "$D" "$N" "$Y" "$N" "$D" "$N"
    ...
    # Opciones del módulo
    printf " [%b01%b] ⚡ GENERAR CUENTA DEMO TEMPORAL                ⚡ %b(AUTOMÁTICO)%b\033[K\n" "$Y" "$N" "$C" "$N"
    echo ""
    printf "%b─────────────────────────────────────────────────────────────────────────────%b\033[K\n" "$D" "$N"
    ...
    read -t 1 -p "$(echo -e " ${C}KIRA@Servidor:~/Usuarios/Demo$ ${N}${W}► Opción: ${N}\033[K")" op
